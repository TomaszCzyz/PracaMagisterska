...
\chapter{Testy numeryczne}
    ...
    W tym rodziale porównamy działanie algorytmów $\varphi^{KP}$ i $\varphi^{MP}$ w praktyce. Oba algorytmy zostały zaimplementowane z użyciem języka Python, wykorzystając m.in. popularne biblioteki: numpy, scipy czy matplotlib. Pewnym ograniczeniem wynikającym z zastosowania tych modułów jest brak natywnego wsparcia dla precyzji większej niż $\mathrm{float64}$, która gwarantuje dokładność na poziomie okolo $15$-ego miejsca po przecinku. Okazało się to szczególnie ważne dla algorytmu $\varphi^{KP}$ ze względu na wybór bardzo małych przedziałów do interpolacji wielomianowej. Zauważmy, że dla stałych zdefiniowanych jak w algorytmie, przy danej precyzji $\eta=10^{-15}$ nie ma sensu brać $m$ większego niż
    \begin{equation*}
        \frac{h^{r+\varrho}}{r+1} \geq \eta \Longrightarrow m \leq T((r+1)\eta)^{-\frac{1}{r+\varrho}}.
    \end{equation*}
    ...
    \begin{flalign*}
        \qquad f_{2}(x) = \begin{cases}
            ...
            \bar{f}_{2}(x+0.5), & \text{ gdy } x \in [t_{f}, T],\end{cases} &&
    \end{flalign*}
    gdzie $\displaystyle \bar{f}_{2}(x) = 1 - \frac{2}{\pi} \sum_{n=1}^{3} \frac{1}{n} \sin(n \pi x)$, $T = 3\frac{1}{2}$ i $t_{f} \in [\frac{T}{2} - \frac{1}{10}, \frac{T}{2} + \frac{1}{10}]$.
    ...
    Osobliwość w podanych funkcjach występuje w punkcie $t_{f}$. Podczas testów punkt ten był losowany z podanych przedziałów. Dzięki temu zmniejszamy liczbę trywialnych przypadków, takich jak położenie osobliwości węźle czy w jego pobliżu (aprkosymacja funkcją stałą w $\varphi^{KP}$).

    Obie funkcje są okresowe oraz należą zarówno do klasy $\F_{r, \varrho}$, jak i $\G$. jednak dla funkcji $f_{2}$ nie zachodzi $\Delta_{f_{2}}^{(0)} = 0$. Algorytmy były testowane w normach $L^{2}$ i $L^{\infty}$. Dla~każdego zestawu danych wejściowych, tj. dla określonego $m$, $\delta$ i $\varrho$, błąd najgorszego przypadku obliczaliśmy jako maksimum z kilkuset uruchomień algorytmu. W ramach obliczania jednego błędu $t_{f}$ było ustalone.
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=.8\linewidth]{Ex4/algMPp2r4.jpg}
        \caption{zależność błędu od $m$ aproksymacji algorytmu $\varphi^{MP}$ dla funkcji $f_{1}$ w normie $L^{2}$.}
        \label{rys:1:algMP_p2}
    \end{figure}
    \begin{figure}[h]
        \centering
        \includegraphics[width=.8\linewidth]{Ex3/algMPpInf.jpg}
    ...
    Aby potwierdzić, że za wyniki niezgodne z oczekiwaniami odpowiada precyzja obliczeń, zaimplementowaliśmy część algorytmu z użyciem bibliteki mpmath. Pozwala ona na przeprowadzenie obliczeń na liczbach o dowolnej precyzji. W bibliotece mpmath dokładność obliczeń ustawia się parametrem \textit{dps}, oznaczającego \textit{miejsca dziesiętne}(\textit{ang. decimal places}). Zastosowanie danej precyzji odpowiada użyciu typu \textit{float} z matysą rezprezentowaną na okolo $3.33 \cdot \mathrm{dps}$ bitach. Z tą zmianą wiążą się dwie ważne kwestie: wydłużenie czasu obliczeń(mniejsze niż możnaby się spodziewać) oraz brak kompatybilności z innymi bibliotekami (numpy, itp.).
    Z początku zwiększyliśmy dokładność obliczeń w kroku pierszym algorytmu, jednak nie przyniosło to oczekiwanych efektów. Po wnikliwej analizie okazało się, że drugim problemem jest prawie zerowa (zerowa przy niższej precyzji) zmiana wartości funkcji na bardzo małych przedziałach, podowodująca duże skoki w wartościach różnic dzielonych. Dopiero zwiększenie dokładności otrzymywanych wartości aproksymowanej funkcji rozwiązało problem. Rysunek \ref{rys:3:diff_dps} przedstawia zależność błędu od $m$ przy zmieniającej się precyzji obliczeń dla informacji bez szumu. Zaznaczmy, zwiększonej dokładności używamy tylko do lokalizacji punktu osobliwego oraz dodania nowych punktów w kroku drugim algorytmu. Końcowa aproksymacja nadal bazuje na 'float64'.

    \begin{figure}[h] \label{rys:3:diff_dps}
        \centering
        \includegraphics[width=.98\linewidth]{Ex4/errors_for_diff_dps.png}
        \caption{$-\log_{10}(err)$ vs $log_{10}(m)$ dla funkcji $f_{2}$ z $r = 4$ oraz $\delta = 0$.}
    \end{figure}
    
    Po wprowadzeniu omówionych usprawnień algortm zachowywał wszystkie teoretyczne własności dla informacji nie zburzonej, jednak....
    % , widoczne na rysunkach [ref] i [ref].
    
    \begin{figure}[h!]
        \centering
        \includegraphics[width=.8\linewidth]{Ex3/algKPp2r2.jpg}
    \end{figure}
    \begin{figure}[h!]
        \centering
        \includegraphics[width=.8\linewidth]{Ex3/algKPpInfr4.jpg}
    \end{figure}
    \begin{figure}[h!]
        \centering
        \includegraphics[width=.8\linewidth]{Ex3/algMPpInf.jpg}
    ...
    \begin{figure}[h!]
        \includegraphics[width=.8\linewidth]{Ex4/algMPpInfr4.jpg}
    \end{figure}
