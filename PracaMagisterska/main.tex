...
\section{Analiza algorytmu opartego o wielomiany Lagrange'a}
    ...
    \begin{proof}
        ...
        \begin{equation} \label{eq:7}
            \sup _{t \in[a, b]}\left\|g(t)-w_{g}^{s}([a, b])(t)\right\| \leq C\left(\sup _{t \in\left[\hat{t}_{g}, b\right]}\left\|s_{g}(t)\right\|+\bar{h}^{\min (s+1, r+\varrho\}}\right)
        \end{equation}
        Niech
        ...
        \end{equation}
        wtedy $\tilde{g} \in G_{reg}^{r, \varrho}([a, b])$. Niech $t_{k}=a+(b-a) k / s$, $k=0,1, \ldots, s$ będą węzłami dla interpolacji $w_{g}^{s}([a, b])$ na przedziale $[a, b]$. Zdefiniujmy
        ...
        \begin{equation*} \label{eq:12}
            \begin{aligned}
                ...
                =& \1_{\left[\hat{t}_{g}, b\right]}(t) s_{g}(t)+\left(\tilde{g}(t)-w_{\tilde{g}}^{s}([a, b])(t)\right)-\sum_{i=j^{*}}^{s} s_{g}\left(t_{i}\right) \Phi_{i}(t)
            \end{aligned}
        \end{equation*}
        ...
        \begin{equation} \label{eq:14}
            \sup _{t \in[a, b]}\left\|g(t)-w_{g}^{s}([a, b])(t)\right\| \leq C\left(\sup _{t \in\left[a, \hat{t}_{g}\right)}\left\|s_{g}(t)\right\|+\bar{h}^{\min \{s+1, r+\varrho\}}\right)
        \end{equation}
    ...
    \end{lemma}
    \begin{proof}
        Niech $\tilde{g} \in G_{reg}^{r, \varrho}([a, b])$ będzie dane jak w \eqref{eq:8}. Ponieważ $\hat{t}_{g}>\bar{a}$, otrzymujemy, że $\tilde{g}(t)=g(t)$ dla wszystkich $t \in[a, \bar{a}]$ oraz
        \begin{equation*}
    ...
    \end{cor}
    \begin{proof}
        Niech $\tilde{g} \in G_{reg}^{r, \varrho}([a, b])$ będzie dana jak w \eqref{eq:15}. Ponieważ $\hat{t}_{g} \leq \bar{a}$, to zachodzi $\tilde{g}(t)=g(t)$ dla $t \in[\bar{a}, b]$ oraz
        \begin{equation*}
    ...
    \end{stw}
    \begin{proof}
        Skoro $\hat{t}_{g} \notin (a, b)$, to funkcja $g$ jest regularna na $[a, b]$. Stąd dla $t \in[a,b]$ mamy
        \begin{equation*}
            \begin{aligned}
    ...
    \begin{proof}
        ...
        \end{equation}
        gdzie $M$ zależy tylko od globalnych parametrów $G_{reg}^{r, \varrho}([a, b])$. Co więcej, używając form Lagrange'a $w_{g}^{s}([\bar{a}, \bar{b}])$ i $w_{\tilde{g}}^{s}([\bar{a}, \bar{b}])$ na przdziale $[\bar{a}, \bar{b}]$, otrzymujemy dla wszystich $j=0,1, \ldots, s$ i $t \in[a, b]$, że
        \begin{equation*}
            \begin{aligned}
        ...
        Dodatowo, ponieważ $\sum_{k=0}^{s}\left|\Phi_{k}^{(j)}(t)\right|=\mathcal{O}\left(\bar{h}^{-j}\right)$ dla $t \in[a, b]$, to
    ...
    \begin{lemma} \label{lem:3}
        ...
        \begin{equation*}
            \hat{t}_{g} \in(a, a+\delta] \cup[b-\delta, b) \longrightarrow \sup _{t \in[a, b]}\left\|\left(w_{g}^{s}([a, b])\right)^{(j)}(t)\right\| \leq C\left(1+\delta \cdot \bar{h}^{-j}\right)
        \end{equation*}
    \end{lemma}
    \begin{proof}
        Załóżmy, że $\hat{t}_{g} \in (a, a+\delta]$. Weżmy $\tilde{g}$ zdefiniowaną jak w \eqref{eq:15}. Wtedy dla $t \in[a, b]$ zachodzi
        ...
        \end{equation}
        gdzie $j^{*}$ jest zdefiniowana jak w \eqref{eq:9} i $\bar{C}$  zależy tylko od parametrów klasy $G^{r, \varrho}([a, b])$. Teza wynika z \eqref{eq:22} i \eqref{eq:23}. Jeżeli $\hat{t}_{g} \in[d-\delta, d)$, wtedy bierzemy $\tilde{g}$ zdefiniowaną w \eqref{eq:15} i postępujemy analogicznie.
    \end{proof}
