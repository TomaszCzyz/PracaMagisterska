...
\section{Informacja, algorytm, aproksymacja}
    ...
    Zakładamy, że $N(f)$ jest niepuste dla wszystkich $f \in F$. Każdy element $y \in N(f)$ będziemy nazywać \textit{informacją o $f$}.
    Zauważmy, że jeżeli zbiór $N(f)$ ma dokładnie jeden element dla wszystkich $f \in F$, to informacja $N$ jest \textit{dokładna}. W przypadku, gdy istnieje $f$ dla którego $N(f)$ ma przynajmniej dwa elementy, wtedy informacja jest \textit{niedokładna(częściowa)}.
