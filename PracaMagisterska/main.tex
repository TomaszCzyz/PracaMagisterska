...
\section{Informacja, algorytm, aproksymacja}
    ...
    \end{equation*}
    Algorytm na wejściu otrzymuje inforację jako dane, innymi słowy, aproksymacją $S(f)$ jest $\varphi(y)$, gdzie $y$ jest informacją o $f$. Błąd aproksymacji zdefiniownay jest jako róznica $\| S(f) - \varphi(y) \|$, gdzie $\|\cdot\|$ jest normą w przestrzeni $G$.

    Dodatkowo, możemy wydzielić dwie klasy informacji ze względu na sposób wybóru kolejnych wartości funckji. Jeżeli informacja jest niezależna od $f$ \textit{inforamcja nieadaptacyjna} 
...
\begin{equation*}
    e^{wor}_{p}(\phi, N, E) = \sup_{f \in E} \sup_{y \in N(f)} \|f - \varphi(y) \|_{L^p}
\end{equation*}
...
W tej pracy porównujemy dwa algorytmy aprksymujęce funkcje z osobliwością, które osiągają to samo, optymalne organiczenie na minimalny błąd najgorszego przypadku, jednak tylko w przypadku jednego z nich w anaizie zostało uwzgędione zaburzenie. W celu dokładnego sprecyzowania ograniczeń algorytmów, w następnym rozdziale szczegółowo definiujemy interesującą nas klase funkcji.
